...
<head>
    ...
    */
        ...
        */
       $dbh = new DB('_s0564294__mockupdatadb', 'db.f4.htw-berlin.de:3306', 's0564294', $password);
...
<body>
   <?php
      ...
      elseif ($_POST) {
            /*
             * hier werden die über das Formular gesendeten Daten ausgewertet; 2 Fälle: wird die id mitgesendet, dienen die übersendeten Daten der Änderung des entsprechenden Datensatzes wird die id nicht mitgeliefert, dienen die Daten dem Einfügen eines neuen Datensatzes in die Datenbank
             */
            ...
            /*hier müsste eingentlich noch getestet werden ob alle Eingaben vollständig sind (siehe Übung 7)*/
             if(isset($_POST['id'])) {
                $id = filter_var($_POST['id'], FILTER_SANITIZE_NUMBER_INT);
             }
             if(isset($id)) { //aktualisieren
                debug_to_console($vorname.', '.$nachname.', '.$email.', '.$ipnr.', '.$id);
                $dbh->edit(array($vorname, $nachname, $email, $ipnr, $id));
                $message = 'Teilnehmerin geändert';
             }
             else { //hinzufügen
       ...
       //$dbh-> edit(array("Jan", "Justermann", "jan@justermann.de", "6789", "51"));
         ...
         <div class="panel-body">
            ...
            <form role="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST">
             ...
             -->
                 ...
                 <input class="form-control" type="text" name="ipnr" value="<?php echo $teilnehmerin['vorname'];?>" />

                 <input type="hidden" name="id" value="<?php echo $teilnehmerin['id'];?>"/>
                 <button class="btn btn-primary btn-block" type="submit">Aktualisieren</button>
            </form>
